...
- [ ] **SELECT * を使っていないか？**
  - 必要な列だけを明示しているか
  - スキーマ変更で壊れるリスクはないか
  - 不要な大きいカラム（BLOB等）を取得していないか
  - カバリングインデックスの機会を逃していないか

**参照:**
- [knowledge/query-antipatterns.md](../knowledge/query-antipatterns.md#implicit-columns暗黙の列)
- [tasks/read-execution-plan.md](./read-execution-plan.md)
...
- [ ] **1つのクエリで複数の目的を達成しようとしていないか？（Spaghetti Query）**
  - 複数のLEFT JOINで多対多の関係を結合していないか
  - 意図しないデカルト積が発生していないか
  - DISTINCTで重複を誤魔化していないか
  - クエリを分割できないか

**参照:** [knowledge/query-antipatterns.md](../knowledge/query-antipatterns.md#spaghetti-queryスパゲッティクエリ)
...
### 10. データモデル（スーパーソルジャー病）

#### チェックポイント

- [ ] **そもそもテーブル設計おかしくない？**
  - 毎回結合+集約が必要なクエリになっていないか
  - SQLで無理に頑張っていないか
  - モデルを変えればシンプルになるのではないか

- [ ] **結合+集約が毎回必要なクエリ → 集計列追加を検討**
  ```sql
  -- ❌ 毎回結合+集約
  ...
  -- ✅ モデル変更: Ordersにitem_count列を追加
  ALTER TABLE Orders ADD COLUMN item_count INT DEFAULT 0;
  SELECT order_id, item_count FROM Orders;
  ```

- [ ] **頻繁に実行されるクエリか？**
  - 頻繁に実行される → 集計列追加の効果大
  - たまにしか実行されない → そのままでも許容

- [ ] **更新頻度とリアルタイム性のトレードオフ**
  - 更新頻度が低い（日次バッチ等） → 集計列追加が有効
  - 更新頻度が高い（リアルタイム） → 更新コストとのトレードオフを慎重に検討
  - リアルタイム性が必要 → ビューまたはキャッシュを検討
...
**参照:**
- [knowledge/anti-patterns.md](../knowledge/anti-patterns.md#スーパーソルジャー病) - スーパーソルジャー病の詳細
...
- [ ] **インデックスが効く条件を満たすか？**
  - カーディナリティが高い（値のばらつきが大きい）か
  - 選択率が低い（5〜10%以下に絞り込める）か
  - インデックスが定義されているか

- [ ] **インデックスが効かない5パターンに該当しないか？**
  1. LIKE中間・後方一致（`LIKE '%keyword%'`, `LIKE '%keyword'`）
  2. 列を加工（計算・関数）（`WHERE YEAR(date) = 2024`）
  3. 否定形（`WHERE status <> 'deleted'`, `NOT IN`）
  4. IS NULL（実装による）
  5. 選択率が高い（90%ヒット等）

- [ ] **複合インデックスの順序は正しいか？**
  - WHERE句の条件がインデックスの先頭列から順に使われているか
  - 先頭列が条件にないとインデックスが効かない

- [ ] **カバリングインデックスで最適化できないか？**
  - SELECT句の列もインデックスに含めることで、テーブル本体へのアクセスを回避できないか
...
カーディナリティは高い？（値のばらつき大）
├─ NO  → インデックスは効かない → UI設計/データマートで対処
└─ YES → 次へ

選択率は低い？（5〜10%以下に絞り込める）
├─ NO  → インデックスは効かない → UI設計で範囲を狭める
└─ YES → 次へ

効かない5パターンに該当？
├─ YES → クエリを書き換える（列を加工しない、前方一致にする等）
└─ NO  → 次へ

複合インデックスの順序は正しい？
├─ NO  → インデックスの順序変更 or WHERE句調整
└─ YES → OK

カバリングインデックスで最適化可能？
├─ YES → SELECT句の列をインデックスに追加
└─ NO  → 現状のまま
```
